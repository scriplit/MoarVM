...
#define CLASS_KEY(c) ((MVMint64)PMC_data(STABLE_PMC(c)))

/* Helper to make an introspection call, possibly with :local. */
static MVMObject * introspection_call(MVMThreadContext *tc, MVMObject *WHAT, MVMObject *HOW, STRING *name, MVMint64 local) {
    MVMObject *old_ctx, *cappy;
    
    /* Look up method; if there is none hand back a null. */
    MVMObject *meth = VTABLE_find_method(interp, HOW, name);
    if (PMC_IS_NULL(meth))
        return meth;
...
static MVMObject * index_mapping_and_flat_list(MVMThreadContext *tc, MVMObject *mro, CStructREPRData *repr_data) {
    ...
    MVMint64  current_slot   = 0;
    
    MVMint64 num_classes, i;
    CStructNameMap * result = NULL;
    
    /* Walk through the parents list. */
    MVMint64 mro_idx = VTABLE_elements(interp, mro);
    while (mro_idx)
    {
        ...
        MVMint64  num_parents = VTABLE_elements(interp, parents);
        if (num_parents <= 1) {
            /* Get attributes and iterate over them. */
            MVMObject *attributes = VTABLE_get_pmc_keyed_int(interp, type_info, 1);
            MVMObject *attr_map   = PMCNULL;
            MVMObject *attr_iter  = VTABLE_get_iter(interp, attributes);
            while (VTABLE_get_bool(interp, attr_iter)) {
                /* Get attribute. */
                MVMObject * attr = VTABLE_shift_pmc(interp, attr_iter);
...
static void compute_allocation_strategy(MVMThreadContext *tc, MVMObject *repr_info, CStructREPRData *repr_data) {
    ...
    /* Otherwise, we need to compute the allocation strategy.  */
        ...
        MVMint64 num_attrs        = VTABLE_elements(interp, flat_list);
        MVMint64 info_alloc       = num_attrs == 0 ? 1 : num_attrs;
        MVMint64 cur_obj_attr     = 0;
        MVMint64 cur_str_attr     = 0;
        MVMint64 cur_init_slot    = 0;
        MVMint64 i;

        /* Allocate location/offset arrays and GC mark info arrays. */
        repr_data->num_attributes      = num_attrs;
        repr_data->attribute_locations = (MVMint64 *) mem_sys_allocate(info_alloc * sizeof(MVMint64));
        repr_data->struct_offsets      = (MVMint64 *) mem_sys_allocate(info_alloc * sizeof(MVMint64));
        repr_data->flattened_stables   = (MVMSTable **) mem_sys_allocate_zeroed(info_alloc * sizeof(MVMObject *));
        repr_data->member_types        = (PMC** )    mem_sys_allocate_zeroed(info_alloc * sizeof(MVMObject *));
        ...
        for (i = 0; i < num_attrs; i++) {
            ...
            MVMint64  type_id      = REPR(type)->ID;
            MVMint64  bits         = sizeof(void *) * 8;
            MVMint64  align        = ALIGNOF1(void *);
            if (!PMC_IS_NULL(type)) {
                /* See if it's a type that we know how to handle in a C struct. */
                MVMStorageSpec spec = REPR(type)->get_storage_spec(interp, STABLE(type));
                if (spec.inlineable == STORAGE_SPEC_INLINED &&
                    ...
                    if (REPR(type)->initialize) {
                        if (!repr_data->initialize_slots)
                            repr_data->initialize_slots = (MVMint64 *) mem_sys_allocate_zeroed((info_alloc + 1) * sizeof(MVMint64));
...
static FLOATVAL get_num_at_offset(void *data, MVMint64 offset) {
    void *location = (char *)data + offset;
    return *((FLOATVAL *)location);
}

/* Helper for writing a num at the specified offset. */
static void set_num_at_offset(void *data, MVMint64 offset, FLOATVAL value) {
    void *location = (char *)data + offset;
    *((FLOATVAL *)location) = value;
}

/* Helper for reading a pointer at the specified offset. */
static void * get_ptr_at_offset(void *data, MVMint64 offset) {
    void *location = (char *)data + offset;
    return *((void **)location);
}

/* Helper for writing a pointer at the specified offset. */
static void set_ptr_at_offset(void *data, MVMint64 offset, void *value) {
    void *location = (char *)data + offset;
    *((void **)location) = value;
}

/* Helper for finding a slot number. */
static MVMint64 try_get_slot(MVMThreadContext *tc, CStructREPRData *repr_data, MVMObject *class_key, STRING *name) {
    MVMint64 slot = -1;
    if (repr_data->name_to_index_mapping) {
        CStructNameMap *cur_map_entry = repr_data->name_to_index_mapping;
        while (cur_map_entry->class_key != NULL) {
            if (cur_map_entry->class_key == class_key) {
                MVMObject *slot_pmc = VTABLE_get_pmc_keyed_str(interp, cur_map_entry->name_map, name);
...
static void compose(MVMThreadContext *tc, MVMSTable *st, MVMObject *repr_info) {
    ...
    MVMObject *attr_info = VTABLE_get_pmc_keyed_str(interp, repr_info,
...
static void initialize(MVMThreadContext *tc, MVMSTable *st, void *data) {
    ...
    if (repr_data->initialize_slots) {
        ...
        for (i = 0; repr_data->initialize_slots[i] >= 0; i++) {
            MVMint64  offset = repr_data->struct_offsets[repr_data->initialize_slots[i]];
...
static MVMObject * get_attribute_boxed(MVMThreadContext *tc, MVMSTable *st, void *data, MVMObject *class_handle, STRING *name, MVMint64 hint) {
    CStructREPRData *repr_data = (CStructREPRData *)st->REPR_data;
    CStructBody     *body      = (CStructBody *)data;
    MVMint64           slot;
    ...
    if (slot >= 0) {
        MVMint64 type      = repr_data->attribute_locations[slot] & CSTRUCT_ATTR_MASK;
        MVMint64 real_slot = repr_data->attribute_locations[slot] >> CSTRUCT_ATTR_SHIFT;
...
static void get_attribute_native(MVMThreadContext *tc, MVMSTable *st, void *data, MVMObject *class_handle, STRING *name, MVMint64 hint, NativeValue *value) {
    CStructREPRData *repr_data = (CStructREPRData *)st->REPR_data;
    CStructBody     *body      = (CStructBody *)data;
    MVMint64           slot;
...
static void bind_attribute_boxed(MVMThreadContext *tc, MVMSTable *st, void *data, MVMObject *class_handle, STRING *name, MVMint64 hint, MVMObject *value) {
    CStructREPRData *repr_data = (CStructREPRData *)st->REPR_data;
    CStructBody     *body      = (CStructBody *)data;
    STRING          *type_str  = Parrot_str_new_constant(interp, "type");
    MVMint64            slot;
...
static void bind_attribute_native(MVMThreadContext *tc, MVMSTable *st, void *data, MVMObject *class_handle, STRING *name, MVMint64 hint, NativeValue *value) {
    CStructREPRData *repr_data = (CStructREPRData *)st->REPR_data;
    CStructBody     *body      = (CStructBody *)data;
    MVMint64            slot;
...
static MVMint64 is_attribute_initialized(MVMThreadContext *tc, MVMSTable *st, void *data, MVMObject *ClassHandle, STRING *Name, MVMint64 Hint) {
    die_no_attrs(interp);
}

/* Gets the hint for the given attribute ID. */
static MVMint64 hint_for(MVMThreadContext *tc, MVMSTable *st, MVMObject *class_handle, STRING *name) {
    return NO_HINT;
}
