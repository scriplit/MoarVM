...
static void copy_to(MVMThreadContext *tc, MVMSTable *st, void *src, void *dest) {
    ...
    if (src_body->arg_types) {
        dest_body->arg_types = (MVMint64 *) mem_sys_allocate(sizeof(MVMint64) * (src_body->num_args ? src_body->num_args : 1));
        memcpy(dest_body->arg_types, src_body->arg_types, src_body->num_args * sizeof(MVMint64));
    }
